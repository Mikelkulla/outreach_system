...
async function checkRunningJobs() {
    try {
        ...
        if (result.jobs && result.jobs.some(job => job.status === "running")) {
            ...
            // If no job is selected, show a generic message
            if (!document.getElementById("job_select_step5").value) {
                statusDiv5.textContent = "A job is running. Select a job to view progress.";
            }
        }

        // Check Step 6 running jobs
        ...
        if (result.jobs && result.jobs.some(job => job.status === "running")) {
            ...
            // If no job is selected, show a generic message
            if (!document.getElementById("job_select_step6").value) {
                statusDiv6.textContent = "A job is running. Select a job to view progress.";
            }
        }
        ...
        if (result.jobs && result.jobs.some(job => job.status === "running")) {
            ...
            if (!document.getElementById("job_select_step7").value) {
                statusDiv7.textContent = "A job is running. Select a job to view progress.";
            }
        }
    } catch (error) {
